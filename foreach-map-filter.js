...
function showFirstAndLast(arr){
    const result = [];
    arr.forEach(n => result.push(n[0] + n[n.length - 1]))
    return result;
}
...
function find(arr, searchValue) {
    return arr.filter(n => n === searchValue)
    [0];
}
...
function findInObj(arr, key, searchValue) {
    return arr.filter(n => n[key] === searchValue)
    [0];
}
...
function removeVowels(str) {
    ...
    .filter(letter => 'aeiou'.indexOf(letter) === -1)
    .join('');
}
